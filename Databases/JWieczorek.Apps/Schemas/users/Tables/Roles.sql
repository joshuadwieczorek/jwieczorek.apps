...
(
	...
	,	CONSTRAINT [AK_Users_Roles_Name] UNIQUE ([Name])
	,	CONSTRAINT [CK_Users_Roles_Status_Type] CHECK ([dbo].[fnTypeValueExits]('users', 'status_type', [Status_Type]) = 1)
	,	PERIOD FOR SYSTEM_TIME ([Lifetime_Start], [Lifetime_End])
)
WITH (SYSTEM_VERSIONING = ON (HISTORY_TABLE = [users].[RolesHistory]));