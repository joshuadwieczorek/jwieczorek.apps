CREATE TABLE [users].[Roles_Map]
(
		[ID]			[dbo].[UUID_Type]		NOT NULL	DEFAULT NEWID()
	,	[Role_ID]		[dbo].[UUID_Type]		NOT NULL
	...
	,	CONSTRAINT [PK_Users_Roles_Map_ID] PRIMARY KEY ([Role_ID], [User_ID])
	,	CONSTRAINT [PK_Users_Roles_Map_Role_ID] FOREIGN KEY ([Role_ID]) REFERENCES [users].[Roles]([ID])
	,	CONSTRAINT [PK_Users_Roles_Map_User_ID] FOREIGN KEY ([User_ID]) REFERENCES [users].[Users]([ID])
	,	PERIOD FOR SYSTEM_TIME ([Lifetime_Start], [Lifetime_End])
)
WITH (SYSTEM_VERSIONING = ON (HISTORY_TABLE = [users].[RolesMapHistory]));